...
import { Alert, AlertActionCloseButton, AlertGroup, Button, Divider, Dropdown, DropdownItem, DropdownList, Label, Masthead, MastheadBrand, MastheadContent, MastheadMain, MastheadToggle, Menu, MenuContent, MenuList, MenuItem, MenuToggle, MenuToggleElement, Nav, NavExpandable, NavItem, NavList, Page, PageSidebar, PageSidebarBody, Popper, SkipToContent, Tooltip } from '@patternfly/react-core';
...
const AppLayout: React.FunctionComponent<IAppLayout> = ({
  ...
  const masthead = <Masthead>
      ...
      <MastheadContent>
        ...
        zIndex: 10
            ...
            <Popper trigger={<MenuToggle ref={signInToggleRef} onClick={() => setSignInMenuOpen(!signInMenuOpen)} isExpanded={signInMenuOpen} id="sign-in-menu-toggle">
                  Sign in
                </MenuToggle>} popper={<Menu ref={signInMenuRef} id="sign-in-menu" onSelect={() => setSignInMenuOpen(false)}>
                  <MenuContent>
                    <MenuList>
                ...
                </Menu>} isVisible={signInMenuOpen} onDocumentClick={event => {
          if (event && signInToggleRef.current && !signInToggleRef.current.contains(event.target as Node) && signInMenuRef.current && !signInMenuRef.current.contains(event.target as Node)) {
            setSignInMenuOpen(false);
          }
        }} />
            <Button icon={<BellIcon />} variant="plain" aria-label="Alerts" />
            <Button icon={<InfoCircleIcon />} variant="plain" aria-label="Information" />
            {flags.displayMode && <Button icon={theme === 'light' ? <MoonIcon /> : <SunIcon />} variant="plain" onClick={toggleTheme} aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`} style={{
          color: theme === 'dark' ? '#f0ab00' : undefined
        }} />}
            <Dropdown isOpen={userDropdownOpen} onSelect={() => setUserDropdownOpen(false)} onOpenChange={(isOpen: boolean) => setUserDropdownOpen(isOpen)} popperProps={{
          position: 'right'
        }} toggle={(toggleRef: React.Ref<MenuToggleElement>) => <MenuToggle ref={toggleRef} onClick={() => setUserDropdownOpen(!userDropdownOpen)} isExpanded={userDropdownOpen} variant="plain" aria-label="User menu">
                  <div style={{
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
            padding: '0.25rem 0.5rem'
              ...
              <DropdownList>
                <DropdownItem key="ai-admin" icon={<UserIcon />} onClick={() => {
              ...
              color: userProfile === 'AI Admin' ? '#0066cc' : 'inherit'
                ...
                <DropdownItem key="ai-engineer" icon={<UserIcon />} onClick={() => {
              ...
              color: userProfile === 'AI Engineer' ? '#0066cc' : 'inherit'
                ...
                <DropdownItem key="feature-flags" icon={<FlagIcon />} onClick={() => navigate('/feature-flags')}>
                  Feature Flags
                </DropdownItem>
                <DropdownItem key="clear-storage" icon={<TrashIcon />} onClick={handleClearLocalStorage}>
                  Clear local storage
                </DropdownItem>
              </DropdownList>
            </Dropdown>
  ...
  const renderNavItem = (route: IAppRoute, index: number, groupId?: string) => {
    ...
    return <NavItem key={itemId} id={itemId} itemId={itemId} groupId={groupId} isActive={route.path === location.pathname}>
        {(route as any).disabled ? <div style={{
        display: 'flex',
        alignItems: 'center',
        ...
        opacity: 0.5
          ...
          gap: '0.5rem'
              ...
              {(route as any).techPreview && <Label variant="outline" color="orange" isCompact style={{
            fontSize: '10px'
          }}>
                  Tech Preview
                </Label>}
            </span>
          </div> : <NavLink to={route.path} style={{
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'space-between'
      }}>
  ...
  const renderNavGroup = (group: IAppRouteGroup, groupIndex: number, parentGroupId?: string) => {
    ...
    return <NavExpandable key={groupId} id={groupId} groupId={groupId} title={<span style={{
      display: 'flex',
      alignItems: 'center',
      gap: '0.5rem'
    }}>
            {IconComponent && <IconComponent />}
            {group.label}
          </span>} isActive={isGroupActive(group.routes)} style={(group as any).disabled ? {
      color: '#6a6e73',
      opacity: 0.5,
      cursor: 'not-allowed'
    } : undefined}>
  ...
  const Navigation = <Nav id="nav-primary-simple">
  ...
  const PageSkipToContent = <SkipToContent onClick={event => {
  ...
  return <div style={{
    ...
    flexDirection: 'column'
      ...
      <Page mainContainerId={pageId} masthead={masthead} sidebar={sidebarOpen && Sidebar} skipToContent={PageSkipToContent} style={{
      flex: 1,
      minHeight: 0,
      position: 'relative'
    }}><CommentPanel><CommentOverlay />{children}
      </CommentPanel></Page>
    </div>;
};
export { AppLayout };